...
exports.recover = async (req, res) => {
    try {
        ...
        if (!user) return res.status(401).json({ message: 'La direccion de correo electronico' + req.body.email + ' no está asociado con ninguna cuenta. Vuelva a verificar su dirección de correo electrónico e intente nuevamente.'});
        ...
        res.status(200).json({message: 'Se ha enviado un correo electrónico de reinicio a ' + user.email + '.'});
    } catch (error) {
...
exports.reset = async (req, res) => {
    try {
        ...
        if (!user) return res.status(401).json({message: 'El token de restablecimiento de contraseña no es válido o ha expirado.'});
